...
func (apiV1 *ApiV1) ServiceWorkerPost(w http.ResponseWriter, r *http.Request) {

	var ps []models.ServiceWorker
	var err error
	...
	box := models.BoxForServiceWorker(apiV1.obx)
	...
	for _, v := range ps {
		...
		query := box.Query(models.ServiceWorker_.ExtId.Equals(v.ExtId, true))
		ServiceWorkers, err := query.Find()
		query.Close()
...
func (api *ApiV1) ServiceWorkerGet(w http.ResponseWriter, r *http.Request) {
	...
	if fvId == "" {
		...
		query := box.Query(models.ServiceWorker_.ExtId.Equals(fvId, true))
		ps, err = query.Find()
		query.Close()
		if err != nil {
			sa.Status = http.StatusInternalServerError
